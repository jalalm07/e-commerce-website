...
//		product.getCategory().
	...
	public Page<Product> getAllProduct(String category, List<String>colors, 
			...
			Integer minDiscount,String sort, String stock, Integer pageNumber, Integer pageSize, String brand, Integer rating) {
		...
		if(!StringUtils.equalsIgnoreCase("null",brand)){
			products=products.stream().filter(p-> p.getBrand().equalsIgnoreCase(brand)).collect(Collectors.toList());
		}

		if(rating > 0){
			products=products.stream().filter(p-> p.getReviews().size() >= rating).collect(Collectors.toList());
		}
